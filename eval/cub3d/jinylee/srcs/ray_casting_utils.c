...
void	init_ray(t_ray *ray, t_info *info, int ray_id)
{
...
double	cal_wall(t_ray *ray, t_info *info)
{
	...
	ray->tex_num = decide_dir(info->direction);
	if (ray->side == 0)
...
int		cal_tex_x(t_ray *ray, t_info *info)
{
	...
	tex_x = (int)(cal_wall(ray, info) * (double)tex_w);
...
void	fill_buf(t_img *img, t_ray *ray, t_info *info, int x)
{
	...
	while (y < ray->draw_end)
	{
		...
		tex_pos += step;
		info->buf[y][x] =
			info->tex[ray->tex_num][img[0].img_w * tex_y + tex_x];
		y++;
	}
}
