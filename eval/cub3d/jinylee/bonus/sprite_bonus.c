...
void		buf_sprite(t_game *info, int stripe, int i, int y)
{
	...
	if (info->sprite[i].trans_y > 0 && stripe > 0 && stripe < info->map.w
		...
		while (y < info->sprite[i].end_y)
		{
			...
			if ((info->tex[info->sprite[i].num][TEXW * tex_y + tex_x] &
			0x00FFFFFF) != 0)
				info->buf[y][stripe] =
				distance_shade(info->tex[info->sprite[i].num]
				[TEXW * tex_y + tex_x], info->sprite[i].dist);
			y++;
		}
}
...
void		ft_sprite(t_game *info)
{
	...
	{
		if (info->sprite[i].del == 1)
		{
			i++;
			continue;
		}
		sprite_trans(info, i);
		sprite_init(info, info->sprite, info->sprite[i].order);
		sprite_draw(info, info->sprite, info->sprite[i].order);
		i++;
	}
}
